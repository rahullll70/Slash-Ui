...
export default function ArcSlider() {
    ...
    const computeTransform = useCallback((slideIndex, scrollOffset, winW, winH) => {
        const centerX = winW / 2;
        const arcBaselineY = winH * 0.45;
        let wrappedOffsetX = (((slideIndex * SLIDE_GAP - scrollOffset) % trackWidth) +
            trackWidth) %
            trackWidth;
        if (wrappedOffsetX > trackWidth / 2)
            wrappedOffsetX -= trackWidth;
    ...
    useEffect(() => {
        ...
        const tick = () => {
            ...
            const { w, h } = getDims();
            const newTransforms = SLIDES.map((_, i) => computeTransform(i, scrollCurrentRef.current, w, h));
    ...
    return (_jsxs("div", { ref: containerRef, className: "relative w-full h-screen overflow-hidden bg-white", onTouchStart: onTouchStart, onTouchMove: onTouchMove, children: [_jsx("style", { children: `@import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&display=swap');` }), SLIDES.map((slide, i) => {
                const t = transforms[i];
                if (!t)
                    return null;
                return (_jsx("div", { className: "absolute will-change-transform flex items-center justify-center", style: {
                        left: t.x,
                        top: t.y,
                        width: t.width,
                        height: t.height,
                        zIndex: t.zIndex,
                    }, children: _jsx("img", { src: slide.src, alt: slide.title, className: "w-full h-full object-cover rounded-[4px]", style: { boxShadow: "0 10px 30px rgba(0,0,0,0.1)" }, onError: (e) => {
                            console.error(`Image not found: ${slide.src}`);
                            e.target.style.background = "#eee";
                        } }) }, i));
            }), _jsx("p", { className: "absolute left-1/2 -translate-x-1/2 pointer-events-none z-[1000] text-center w-full text-black tracking-widest", style: {
                    bottom: "15vh",
                    fontFamily: "'Bebas Neue', sans-serif",
                    fontSize: "5rem",
                    letterSpacing: "2px",
                }, children: (_b = (_a = SLIDES[activeIndex]) === null || _a === void 0 ? void 0 : _a.title) !== null && _b !== void 0 ? _b : "" })] }));
}
