...
const Cta = () => {
  ...
  const splitText = (text: string) => {
    return text.split('').map((char, i) => (
      <span
        key={i}
        className='inline-block translate-y-full opacity-0 footer-char'
      >
        {char === ' ' ? '\u00A0' : char}
      </span>
    ));
  ...
  return (
    <>
      <footer
        ref={footerRef}
        className='relative overflow-hidden text-white border-neutral-800 '
      >
        ...
        <div className='relative px-4 pt-20 pb-40 mx-auto overflow-hidden max-w-7xl sm:px-6 lg:px-8'>
          ...
          <div className='relative z-10 text-center'>
            ...
            <div className='flex justify-center overflow-x-hidden translate-y-4 opacity-0 footer-fade'>
              <Link
                href='/pricing'
                className='relative flex items-center gap-2 py-4 overflow-hidden font-semibold tracking-widest text-white transition-all duration-300 border rounded-full cursor-pointer group px-15 hover:text-black font-hoshiko hover:bg-red-500 hover:scale-105 active:scale-95'
              >
                Slash Now
              </Link>
            </div>
