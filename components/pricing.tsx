  ...
  return (
    ...
    >
      ...
      <div className='relative z-10 flex flex-col flex-grow'>
        {/* Tier Header */}
        <h3 className='mb-2 text-lg font-semibold font-switzer text-brand-light/80'>
          {tier}
        </h3>

        {/* Big Bold Price */}
        <div className='mb-12 text-6xl font-black tracking-tight font-switzer md:text-7xl text-brand-light'>
          {price}
        </div>
        ...
        <div className='flex-grow mb-12 space-y-2'>
          {features.map((feature, index) => (
            <div
              key={index}
              className='flex items-center gap-4 text-sm font-inter md:text-md text-brand-light/90'
            >
              <Check
                size={18}
                className='text-brand-light shrink-0 stroke-[2.5]'
              />
              <span>{feature}</span>
            </div>
          ))}
      ...
      <div className='relative z-10'>
        {href ? (
          <Link
            href={href}
            className='block w-full py-4 text-sm text-center text-black transition-all duration-300 rounded-full shadow-lg font-switzer bg-brand-light hover:opacity-90'
          >
            Get Instant Access
          </Link>
        ) : (
          <div className='block w-full py-4 text-sm text-center rounded-full cursor-not-allowed font-switzer text-brand-light/60 bg-brand-light/10'>
            Coming Soon
          </div>
        )}
      </div>
  ...
  return (
    <section className='w-full px-6 py-24 font-switzer'>
      ...
      <div className='max-w-3xl mx-auto'>
        ...
        >
          <h2 className='block mb-3 overflow-hidden'>
            <motion.span
              variants={cardVariants}
              className='block text-5xl font-black tracking-tighter md:text-5xl text-brand-light'
            >
          ...
          <motion.p
            variants={cardVariants}
            className='max-w-lg text-sm leading-relaxed text-zinc-400 font-inter '
          >
